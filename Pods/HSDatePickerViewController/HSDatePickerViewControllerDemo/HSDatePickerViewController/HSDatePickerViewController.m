...
- (void)viewDidLoad {
    ...
    [self pickerView:self.pickerView didSelectRow:[self defaultRowValueForComponent:YearPicker] inComponent:YearPicker];
    ...
    NSDateComponents *components = [[NSCalendar currentCalendar] components:(NSCalendarUnitMonth | NSCalendarUnitDay) fromDate:self.date];
    [self setPickerView:self.pickerView rowInComponent:MonthPicker toIntagerValue:[components month] decrementing:NO animated:NO];
    [self setPickerView:self.pickerView rowInComponent:DayPicker toIntagerValue:[components day]  decrementing:NO animated:NO];
...
- (NSDate *)maxDate {
    if (!_maxDate) {
        _maxDate = [self dateForRow:[self pickerView:self.pickerView numberOfRowsInComponent:YearPicker] - kBufforRows];
    }
    return _maxDate;
}
...
- (NSInteger)maxRowIndex {
    if (_maxRowIndex == -1) {
        if ([self.maxDate compare:[self dateForRow:[self pickerView:self.pickerView numberOfRowsInComponent:YearPicker] - kBufforRows]] == NSOrderedAscending) {
            _maxRowIndex = [self defaultRowValueForComponent:YearPicker] + [self daysBetweenDate:self.date andDate:self.maxDate];
        }
        else {
            _maxRowIndex = [self pickerView:self.pickerView numberOfRowsInComponent:YearPicker] - kBufforRows;
        }
    }
    return _maxRowIndex;
}
...
- (NSInteger)minRowIndex {
    if (_minRowIndex == -1) {
        if ([self.minDate compare:[self dateForRow:kBufforRows]] == NSOrderedDescending) {
            _minRowIndex = [self defaultRowValueForComponent:YearPicker] + [self daysBetweenDate:self.date andDate:self.minDate];
        }
        else {
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    NSInteger firstComponentRowValue = [pickerView selectedRowInComponent:YearPicker];
    //If picker values are to close to bufforValue scroll it back
    if (firstComponentRowValue <= self.minRowIndex) {
        NSDateComponents *components = [[NSCalendar currentCalendar] components:(NSCalendarUnitMonth | NSCalendarUnitDay) fromDate:self.minDate];
        [self setPickerView:self.pickerView rowInComponent:MonthPicker toIntagerValue:[components month] decrementing:NO animated:YES];
        [self setPickerView:self.pickerView rowInComponent:DayPicker toIntagerValue:[components day]  decrementing:NO animated:YES];
    }
    if (firstComponentRowValue >= self.maxRowIndex) {
        NSDateComponents *components = [[NSCalendar currentCalendar] components:(NSCalendarUnitMonth | NSCalendarUnitDay) fromDate:self.maxDate];
        [self setPickerView:self.pickerView rowInComponent:MonthPicker toIntagerValue:[components month] decrementing:YES animated:YES];
        [self setPickerView:self.pickerView rowInComponent:DayPicker toIntagerValue:[components day]  decrementing:YES animated:YES];
    }
}
...
- (void)setPickerView:(UIPickerView *)pickerView rowInComponent:(NSInteger)component toIntagerValue:(NSInteger)value decrementing:(BOOL)decrementing animated:(BOOL)animated {
    if (decrementing) {
        ...
        if (!valueSetted && component == DayPicker) {
            [pickerView selectRow:[pickerView selectedRowInComponent:MonthPicker] - 1 inComponent:MonthPicker animated:animated];
            if ([[self pickerView:pickerView attributedTitleForRow:[pickerView selectedRowInComponent:MonthPicker] forComponent:MonthPicker].string integerValue] == 0) {
                [pickerView selectRow:[pickerView selectedRowInComponent:YearPicker] - 1 inComponent:YearPicker animated:animated];
            }
        }
    } else {
        BOOL valueSetted = NO;
...
- (NSDate *)dateForRow:(NSInteger)row {
    row = row - [self defaultRowValueForComponent:YearPicker];
...
- (NSDate *)dateWithSelectedTime {
    NSDate *date = [self dateForRow:[self.pickerView selectedRowInComponent:YearPicker]];
    NSInteger hour = [[self pickerView:self.pickerView attributedTitleForRow:[self.pickerView selectedRowInComponent:MonthPicker] forComponent:MonthPicker].string integerValue];
    NSInteger minute = [[self pickerView:self.pickerView attributedTitleForRow:[self.pickerView selectedRowInComponent:DayPicker] forComponent:DayPicker].string integerValue];
...
- (IBAction)jumpToPreviousMonth:(id)sender {
    NSDate *date = [self dateForRow:[self.pickerView selectedRowInComponent:YearPicker]];
    NSInteger row = [self.pickerView selectedRowInComponent:YearPicker] - [self daysInMonth:date];
    [self.pickerView selectRow:row inComponent:YearPicker animated:YES];
    [self pickerView:self.pickerView didSelectRow:row inComponent:YearPicker];
}
